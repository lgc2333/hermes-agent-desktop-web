...
const remoteBtn = await waitFor(sessionId, `(() => {
  const overlay = [...document.querySelectorAll('.fixed.inset-0')].find(e => (e.className || '').toString().includes('z-(') && !(e.className || '').toString().includes('onboarding'))
  return overlay ? overlay.innerText.includes('Retry') : null
})()`, 45000, 'remote-failure overlay')
...
const body1 = await evalIn(sessionId, `document.body.innerText.includes('Repair install') || document.body.innerText.includes('Open logs')`)

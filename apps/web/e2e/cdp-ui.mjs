...
await waitFor(sessionId, '!!window.hermesDesktop && document.body.innerText.includes("Gateway") && document.body.innerText.includes("ready")', 60000, 'reload')
...
const signInText = await evalIn(sessionId, `
...
`, 45000, 'oauth connected UI')
pass('ui connected', String(connected))

// 注册表反映 OAuth 模式（token 清空）
const registry = await evalIn(sessionId, `
  JSON.parse(window.localStorage.getItem('hermes-web.connections.v1'))
`)
pass('registry oauth', 'authMode=' + registry.connections[0].authMode + ' token=' + JSON.stringify(registry.connections[0].token))
...
const oauthAfter = await evalIn(sessionId, `
...
// 聊天消息仍在（会话恢复）
const restored = await evalIn(sessionId, `
  (async () => {

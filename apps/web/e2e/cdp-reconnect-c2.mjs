...
execSync(
  `powershell -NoProfile -Command "Get-NetTCPConnection -LocalPort 5180 -State Listen -ErrorAction SilentlyContinue | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force }"`,
)

// 等状态栏变 connecting（确认断连被 UI 感知）
await waitFor(
  sessionId,
  `(document.querySelector('[data-slot="statusbar"]')?.innerText || '').includes('connecting')`,
  20000,
...
const mock = spawn(process.execPath, ['apps/web/dev/mock-gateway.mjs', '5180'], {
  cwd: process.cwd(),
  stdio: 'ignore',
  detached: true,
})
...
await waitFor(
  sessionId,
  `(document.querySelector('[data-slot="statusbar"]')?.innerText || '').includes('ready')`,
  40000,
  'ready again',
)

...
const killPort = (port) => {
  try {
    ...
    console.log('  kill warn:', e.message.slice(0, 120))
  }
}
...
const proxy = spawn(
  'deno',
  ['run', '--allow-net', '--allow-read', '--allow-env', 'apps/proxy/src/main.ts'],
...
const afterRestart = await evalIn(
  sessionId,
  `(() => {
  const btns = [...document.querySelectorAll('button')].map(b => b.textContent.trim())
  return {
...
info(
  'settings after proxy restart',
  'signIn=' + afterRestart.hasSignIn + ' signOut=' + afterRestart.hasSignOut,
)
...
const sess = await evalIn(
  sessionId,
  `window.hermesDesktop.getConnectionConfig().then(c => ({ connected: c.remoteOauthConnected, preview: c.remoteTokenPreview }))`,
)

...
const overlay = await waitFor(sessionId, `(() => {
...
})()`, 45000, 'boot-failure overlay')
console.log('overlay text:', JSON.stringify(overlay))
await new Promise(r => setTimeout(r, 1500))

const btns = await evalIn(sessionId, `(() => {
...
const bodyHas = await evalIn(sessionId, `document.body.innerText.includes('Use local gateway')`)
